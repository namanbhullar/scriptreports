...
(function($){
	...
	$.fn.sendMessagePopUp	=	function(options){
		...
		var setting	=	$.extend({
			...
			if($("#" + setting.overlayerId).length == 0)
			{
			var $overlayer	=	$(setting.overlayerMarkup).attr('id',setting.overlayerId).appendTo('body')
			...
			var $button	=	$("#" + setting.clicker);		
			
			//binding events		
			$button.click(function(){
				setting.beforeOpen();
				
				if(setting.clicker=='submit-report'){
					check	=	true;	
			...
			$overlayer.click(function(){
				...
				document.getElementById('mesaage-sending-form').reset();
	...
	$.fn.PopUp	=	function(options){		
		var setting	=	$.extend({
			...
			var $messageBox	=	$("#" + setting.boxDivId);
...
$(document).ready(function(e) {
	if($("body").find("#add-template").length){$("#add-template").templateList();}
	if($("body").find("#add-script").length){
...
function attachfileSelect(e){
	...
	$("#FilesDiv").append("<input type=\"file\" name=\"attach_file[]\" onchange=\"attachfileSelect(event)\" id=\"MSGfileBtn"+UploadCount+"\" accept=\"application/pdf\" class=\"uploadbutton\">");
	
	$btnWr.find(".btn").attr("onclick","javascript:$('#MSGfileBtn" + UploadCount + "').trigger('click')");
	...
	$crsbtn.click(function(){
		...
		{
			$("#upload-count").html("<i class=\"fa fa-arrow-up\"></i>(" + liLenght + ")");
		}
