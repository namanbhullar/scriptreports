...
<div style="float:left;width:80px;margin-top:-5px;color:rgb(150,150,150); font-size:13px">Powered by</div><div style="float:left;margin-top:3px;">{!!Html::image('public/images/script_reports.png','',array('style'=>'height:15px; width:80px;'))!!}</div><div style="float:right; width:55px; margin-top:-20px">{PAGENO}</div></div>
</htmlpagefooter>
                    		...
                    		<?php if(!empty($left)){ ?>
                            	<?php if(is_array($left)){ $count_value	=	1; ?>
                                	<?php foreach($left as $key=>$value){ ?>
                                    	@if(!empty($value))
                                			<div><b>{!! $label !!} {{$count_value}}:</b> 
                                        &nbsp;{!! $value !!}</div>
                                        @endif
                                    <?php $count_value++; } ?>
                                <?php }else{?>
									<div><b>{!!$label!!}:</b> &nbsp;{{$left}}</div>
                                <?php } ?>
                                        ...
                                        <?php foreach($data[graphs] as $gkey => $graphs) { ?>
                                            <?php if($graphs>1){ ?>
                                                <div class="main_graphdiv" style="float:left;">
                                                    ...
                                                    <div class="title">{!!$graphtitle!!}</div>
                                                    <div class="readBigBG">
                                                        <div class="whiteBG">
					...
					<?php foreach($data['data'] as $category => $questions){	?>
								...
								foreach($questions as $gscore)
                                    ...
                                    <div class="main_graphdiv" style="float:left;">
                                            ...
                                            <div class="rscore"><span style="color:#000;"><?php echo $gainedscore ?></span>/{!!$totalscore!!}</div>
                                    </div>
            ...
            <div class="report_2" style="float:left; padding-left:0px; margin-bottom:5px;">
                <?php if(isset($data['logsys'])){ ?>
					<?php foreach($data['logsys'] as $index => $logsyn) { ?>
                                <?php if(!empty($logsyn)){ ?>
                                    <div><b>{!!$index!!}:</b><br><p>{!! $logsyn !!}</p></div>
                                <?php $logsyspart=true; } ?>
                    <?php } ?>
                <?php } ?>   
            </div>
                ...
                <?php if(isset($data['character_break'])){ ?>
					<?php foreach($data['character_break'] as $index => $charector) { ?>
                                <?php if(!empty($charector)){ ?>
                                    <div><b>{!!$index!!}:</b><br><p>{!!$charector!!}</p></div>
                                <?php $chctrpart=true; } ?>
                    <?php } ?>
                <?php } ?>   
            </div>    
            ...
            <div class="report_2" style="float:left; padding-left:0px; margin-bottom:5px;">
                <?php if(isset($data['notes'])){ ?>
					<?php foreach($data['notes'] as $index => $notes) { ?>
                    		<?php if(!empty($notes)){ ?>
                            	 <div><b>{!!$index!!}:</b><br><p>{!! $notes !!}</p></div>
                            <?php } ?>
                	<?php } ?>
                <?php } ?>
            </div>
            	...
            	<?php if(isset($data['data']) && is_array($data['data'])){ ?>
					<?php foreach($data['data'] as $category => $questions){	?>
                        ...
                        <div style="margin-bottom:10px;width:100%;text-align:left; margin-top:5px; border-top:1px solid #666;border-bottom:1px solid #666;padding:4px 0px;">
                            <div style="float:left; font-size:20px; font-weight:bold;width:400px;margin-right:50px;">{!!$cat->title!!}</div>
                            <div style="float:right; text-align:right; padding-top:5px;"><b>TOTAL SCORE <font color="#D01C1D">{!!$gainedscore!!}</font> out of {!!$totalscore!!}</b></div>
                        </div>
                            <?php 
                                $qcount	=	1;
                            ...
                            ?>
                                <div class="innertag">
                                    <div style="float:left;width:650px;margin-right:10px;">
                                        <b>{!!$qcount!!}. {!!App\Models\Questions::find($id)->title!!} </b>
                                        @if(!empty($question['notes']))
                                            <br/><b>Notes:</b>&nbsp;{!! $question['notes'] !!}
                                        @endif
                                        @if(!empty($question['suggestion']))
                                            <p style="margin-top:5px;"><b>Suggestions:</b>&nbsp;{!!$question['suggestion']!!}</p>
                                        @endif
                                    </div>
                                    <div class="scorespan"> Score {!! $question['score'] !!}</div>
                                </div>
                    		<?php $qcount++; } ?>
              		<?php } ?>  
                 <?php } ?>     
