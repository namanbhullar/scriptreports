       ...
       <?php /*?> Load message Form Inboxdetail<?php */?>
        ...
        <div class="msg_left">
            ...
            <?php  $sender	=	($message->sent_by == $inbox->sender->id) ? $inbox->sender : $inbox->receiver; ?>
                <div class="col-1-1 mrgbt20 messagedetail relative" id="message{{ $message->id}}">
                            ...
                            <?php $request->load('request');?>
                            	@if($request->request)                     	
                                    <div class="col-4 pright inner-request">
                                        ...
                                        @else
                                            <button class="btn btn-primary disabled right mrgrt20 btn-icon fa-check xpul40">{{ ucfirst($request->request->request_status) }}</button>
                                        @endif
                                    </div>
                                @endif
                                @push('script')
