        ...
        <div id="template_view" class="col-1-1 ymrg10">
        	{{Form::open(['route'=>'template.submitreport','id'=>'Create_Report_Form'])}}
            	...
            	<div class="col-1-1">
                  ...
                  @if(auth()->user()->id  != $template->user_id && !is_null($shareData))
                  	<a class="right btn btn-primary xpul20" id="submit-report" href="#submit-report-poopup">{{ trans('lang.save-report') }}</a>
                  @endif
                </div>
                ...
                <div class="col-1-1 ymrg15">
                    ...
                    <div class="right">
                        ...
                        @if(auth()->user()->id == $template->user_id)
                        <a class="btn right btn-icon fa-save btn-primary xpul30 mrgrt30" href="{{ URL::route('template.savePre',['id'=>$tempalte->id]) }}">
                        	Save Template
                        </a>
                                ...
                                <?php if(!empty($leftval) && !is_array($leftval)){?>                                    
                                    <div class="col-1-1 relative">
                                     @if( strtolower($leftval) == "writer" || strtolower($leftval) == "story by" )
                                        <div class="col-20-24 mrgbt15" id="move_{{$idcount}}" data-div="left_{{str_replace(' ','',strtolower($leftval))}}_add">
                                        	{{ Form::label('general_info', $leftval,['class'=>'it']) }}
                                        	{{Form::text('general_info[left]['.$leftval.'][1]','',array('class' => 'text textInput it mrgbt5'))}}
                                        </div>
                                        
                                        <a data-count="1" data-div="left_{{str_replace(' ','',strtolower($leftval))}}_add" class="add_other col-20-24 mrgbt15"><i class="fa fa-plus"></i>&nbsp;Add {{$leftval}}</a>
                                        @else
                            		...
                            		@foreach($leftval as $other)
                                    	 <div class="col-1-1 mrgbt15 relative">
                                            <div class="col-20-24" id="move_{{$idcount}}">
                                                ...
                                                {{Form::text('general_info[left]['.$other.']','',array('class' => 'text textInput it mrgbt5'))}}
                                            </div>
                                            <div class="col-4-24">
                                                <span id="move_{{$idcount}}" title="Remove" class="delete_btn mrgtp25 mrgrt70"><i class="fa fa-trash"></i></span>
                                            </div>
                                ...
                                <?php if(!empty($leftval) && !is_array($leftval)){?>                                    
                                    <div class="col-1-1 relative">
                                     @if( strtolower($leftval) == "writer" || strtolower($leftval) == "story by" )
                                        <div class="col-20-24 mrgbt15" id="move_{{$idcount}}" data-div="left_{{str_replace(' ','',strtolower($leftval))}}_add">
                                        	{{ Form::label('general_info', $leftval,['class'=>'it']) }}
                                        	{{Form::text('general_info[right]['.$leftval.'][1]','',array('class' => 'text textInput it mrgbt5'))}}
                                        </div>
                            		...
                            		@foreach($leftval as $other)
                                    	 <div class="col-1-1 mrgbt15 relative">
                                            <div class="col-20-24" id="move_{{$idcount}}">
                                                {{ Form::label('general_info', $other,['class'=>'it']) }}
                                                {{Form::text('general_info[right]['.$other.']','',array('class' => 'text textInput it mrgbt5'))}}
                                            </div>
                                            <div class="col-4-24">
                                                <span id="move_{{$idcount}}" title="Remove" class="delete_btn mrgtp25 mrgrt70"><i class="fa fa-trash"></i></span>
                                            </div>
            ...
            {!! Html::image("public/images/icons/cancel.png","Close This Dilog Box",["onclick"=>"javascript:$(\"#message-overlayer\").trigger('click')","class"=>"closeMessageBox"]) !!}
                ...
                <div class="col-1-1 pul15">
                    <div class="col-1-1 ymrg10">
                        {{Form::text('script_title','',array('class' => 'text textInput it','placeholder'=>'Script Title', 'required'))}}
                    </div>
                    <div class="col-1-1 ymrg10">
                        {{Form::text('script_draft','',array('class' => 'text textInput it','placeholder'=>'Draft'))}}
                    </div>
                    <div class="col-1-1 ymrg10">
                        {{Form::text('script_date',date('m/d/Y'),array('class' => 'text textInput it datepicker' ,'placeholder'=>'Date','readonly'))}}
                    </div>
                    <div class="col-1-1 ymrg10">
                        {{Form::textarea('script_notes','',array('class' => 'text it textArea', 'rows'=>'2' ,'placeholder'=>'Private Notes'))}}
                    </div>
                </div>
                <div class="col-1-1 xpul20 pulbotm20">
                    <div class="col-1-1">
                        {{-- Form::button('Cancel',array('class'=>'btn btn-icon btn-primary fa-times right','id'=>'cancelreport')) --}}
                        {{Form::submit('Save',array('class'=>'btn btn-primary  right mrgrt15','type'=>'submit'))}}
                    </div>
                </div>
