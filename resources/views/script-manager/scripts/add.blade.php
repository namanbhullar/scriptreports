...
<div class="col-2-3 ymrg20">	
	 <h4 class="BlueRadialHead mrgbt10">{{ strtoupper( 'Documents' ) }}</h4>
      
      <!--- Add Script Section --->
    <div class="col-1-1 mrgbt20" id="script-doc-container">
    	{{Form::label('script',trans('lang.script'),array('class'=>'it mrgbt5'))}}
        <div class="col-2-3">
        	@if($script->documents['Script'])
            	<?php  $dox	=	App\Models\Documents::find($script->documents['Script']); ?>
                {{Form::text('script',$dox->file_name,array('id'=>'script_value','class' => 'text it', 'placeholder' => 'Upload Script',"readonly"=>"readonly"))}}
                {{Form::hidden("all_docs[Script]",$dox->id)}}  
            @else
    ...
    <div class="col-1-1 mrgbt20" id="treatment-doc-container">
        ...
        <div class="col-2-3">
        	@if($script->documents['Treatment'])
            	<?php  $dox	=	App\Models\Documents::find($script->documents['Treatment']); ?>
                {{Form::text('treatment',$dox->file_name,array('id'=>'treatment_value','class' => 'text it', 'placeholder' => 'Upload Script',"readonly"=>"readonly"))}}
                {{Form::hidden("all_docs[Treatment]",$dox->id)}}  
            @else
    ...
    <div class="col-1-1 mrgbt20" id="coverage-container">
        ...
        <div class="col-2-3">
        	@if($script->documents['Script Coverage'])
            	<?php  $dox	=	App\Models\Documents::find($script->documents['Script Coverage']); ?>
                {{Form::text('script_coverage',$dox->file_name,array('id'=>'script_coverage_value','class' => 'text it', 'placeholder' => 'Upload Script Coverage',"readonly"=>"readonly"))}}
                {{Form::hidden("all_docs[Script Coverage]",$dox->id)}}  
            @else
    ...
    <div class="col-1-1 mrgbt20" id="outline-container">
        ...
        <div class="col-2-3">
        	@if($script->documents['Outline'])
            	<?php  $dox	=	App\Models\Documents::find($script->documents['Outline']); ?>
                {{ Form::text('outline',$dox->file_name,array('id'=>'outline_value','class' => 'text it', 'placeholder' => 'Upload Outline' , "readonly" => "readonly")) }}
                 {{Form::hidden("all_docs[Outline]",$dox->id)}}  
            @else
    ...
    <div class="col-1-1 mrgbt20" id="one-sheet-container">
        ...
        <div class="col-2-3">
        	@if($script->documents['One Sheet'])
            	<?php  $dox	=	App\Models\Documents::find($script->documents['One Sheet']); ?>
                {{ Form::text('one-sheet',$dox->file_name,array('id'=>'one-sheet_value','class' => 'text it', 'placeholder' => 'Upload One-Sheet' , "readonly" => "readonly")) }}
                {{Form::hidden("all_docs[One Sheet]",$dox->id)}}  
            @else
    ...
    <div class="col-1-1 mrgbt20" id="charactor-container">
        ...
        <div class="col-2-3">
        	@if($script->documents['Charactor List'])
            	<?php  $dox	=	App\Models\Documents::find($script->documents['Charactor List']); ?>
                {{ Form::text('charactor_list',$dox->file_name,array('id'=>'charactor_list_value','class' => 'text it', 'placeholder' => 'Upload Character List' , "readonly" => "readonly")) }}
              {{Form::hidden("all_docs[Charactor List]",$dox->id)}}  
            @else
        ...
        <div class="col-1-3">
        	<div class="col-1-3 addScript-add-docs">
                ...
                {{Form::file('charactor_list',array('class' => 'filebutton','accept'=>'application/pdf','onchange'=>'fileSelect("charactor_list_value",event); ' , 'id'=>'docs_charactor_list'))}}
            </div>    
...
?>
	@if(count($script->documents))
	@foreach($script->documents as $key => $otherdox)
    
    @if(!in_array($key,$files))
    <div class="col-1-1 mrgbt20" id="ext-other-{{$otherdox}}">
    	{{Form::label("other_docs[$othercount]['name']",$key,array('class'=>'it mrgbt5'))}}
        <div class="col-2-3">
        		<?php  $dox	=	App\Models\Documents::find($otherdox); ?>
                {{ Form::text("other_file_name_$otherdox",$dox->file_name,array('id'=>'other_file_name_'.$otherdox,'class' => 'text it', 'placeholder' => 'Upload Character List' , "readonly" => "readonly")) }}
            
        </div>
        <div class="col-1-3"> 
        	<div class="col-1-3 addScript-add-docs">
                <i class="fa fa-upload" onClick="javascript:$('#docs_other_{{$othercount}}').trigger('click')" title="Uplaod File"></i>
                {{Form::file("other_docs[$otherdox][file]",array('class' => 'filebutton','accept'=>'application/pdf','onchange'=>"fileSelect('other_file_name_$otherdox',event); " , 'id'=>'docs_other_'.$othercount))}}
            </div>    
            <div class="col-1-3 addScript-add-docs iframe-fancybox" href="{{ URL::to('/myaccount/script-manager/my-documents/iframe?script=addScript.js&name=other_file_name_'.$otherdox.'#other_docs_value_'.$otherdox) }}">
                <i class="fa fa-plus"></i>
                {{Form::hidden("other_docs_value_$otherdox",'',array('id'=>"other_docs_value_$otherdox"))}}
                {{Form::hidden("other_docs[$otherdox][name]",$key)}}
                {{Form::hidden("all_docs[$key]",$dox->id)}}
            </div>
            <div class="col-1-3 addScript-add-docs" onClick="javascript:$('#ext-other-{{$otherdox}}').remove()" ><i class="fa fa-trash"></i></div>
        </div>
    </div>
    <?php $othercount++; ?>
    @endif
